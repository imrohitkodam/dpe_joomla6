...
Factory::getApplication()->enqueueMessage(Text::_('RST_STATUS_TRANSLATE'));

$canEdit  	= Factory::getUser()->authorise('status.state', 'com_rsticketspro');
$canChange  = Factory::getUser()->authorise('status.edit.state', 'com_rsticketspro');
...
?>
<form action="<?php echo Route::_('index.php?option=com_rsticketspro&view=statuses'); ?>" method="post" name="adminForm" id="adminForm">
	<?php
	...
	else
	{
		...
		<table class="table table-striped" id="articleList">
			...
			<tr>
				...
				<th><?php echo HTMLHelper::_('searchtools.sort', 'RST_STATUS', 'name', $listDirn, $listOrder); ?></th>
			...
			foreach ($this->items as $i => $item)
			{
				...
				<tr data-draggable-group="1">
					...
					<td>
						...
						if ($canEdit)
						{
							echo HTMLHelper::_('link', Route::_('index.php?option=com_rsticketspro&task=status.edit&id='.(int) $item->id), $this->escape($item->name));
						}
						else
						{
					...
					<td width="1%" nowrap="nowrap" align="center"><?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'statuses.', $canChange && $item->id > 3); ?></td>
