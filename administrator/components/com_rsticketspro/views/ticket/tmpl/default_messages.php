...
if (!$this->isPrint)
{
	if ($this->canViewHistory)
	{
		echo HTMLHelper::_('bootstrap.renderModal', 'rsticketsproHistoryModal', array(
			...
			'backdrop' => 'static'));
		?>
		<a href="#" class="btn btn-secondary" onclick="jQuery('#rsticketsproHistoryModal').modal('show');"><i class="icon-calendar"></i> <?php echo Text::_('RST_TICKET_VIEW_HISTORY'); ?></a>
		<?php
	}
	if ($this->canViewNotes)
	{
		echo HTMLHelper::_('bootstrap.renderModal', 'rsticketsproNotesModal', array(
			...
			'backdrop' => 'static'));
		?>
		<a href="#" class="btn btn-secondary" onclick="jQuery('#rsticketsproNotesModal').modal('show');"><i class="icon-file"></i> <?php echo $this->ticket->notes ? Text::sprintf('RST_TICKET_VIEW_NOTES_NO', $this->ticket->notes) : Text::_('RST_TICKET_VIEW_NOTES'); ?></a>
		<?php
	}
	?>
	...
	<div class="media com-rsticketspro-message<?php echo is_null($user) ? ' alert alert-info' : (RSTicketsProHelper::isStaff($message->user_id) ? ' com-rsticketspro-msg-staff': ' com-rsticketspro-msg-customer'); ?>">
		...
		<div class="media-body">
			...
			if (!is_null($user))
			{
				...
				?>
				<h4 class="media-heading"><?php echo $text; ?><?php echo $submitter ? ' ' . Text::sprintf('RST_TICKET_SUBMITTED_BY', $submitter->name) : ''; ?></h4>
				<?php
			}
			?>
				...
				<div>
					...
					if ($this->canEditMessage($message))
					{
						echo HTMLHelper::_('bootstrap.renderModal', 'rsticketsproMessageModal'.$message->id, array(
							...
							'backdrop' => 'static'));
						?>
						<a class="btn btn-secondary btn-small btn-sm" onclick="jQuery('#rsticketsproMessageModal<?php echo $message->id; ?>').modal('show');" href="#"><i class="icon-edit"></i> <?php echo Text::_('RST_TICKET_EDIT_MESSAGE'); ?></a>
						<?php
					}
